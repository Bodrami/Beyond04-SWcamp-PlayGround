...
/* 설명. 현재의 Controller 클래스에 작성할 핸들러 메소드들이 모두/ first/xxx 의 요청을 받게 될 때 클래스에 어노테이션을 추가할 수 있다. */
...
//    public String registMenu(HttpServletRequest request) {
    ...
    /* 설명. request의 parameter로 넘어오는 값들의 key값과 일치하는 변수명을 작성하고 @RequestParam
    ...
    /* 설명. @RequestParam의 속성들
     *      1. defaultValue: 사용자의 입력 값이 없거나("") request의 parameter 키 값과 일치하지 않는 매개변수 명 사용 시
    ...
    /* 설명. @ModelAttribute 어노테이션을 활용하면 커맨드 객체를 모델에도 담아주며 어트리뷰트의 키값을 지정할 수 있다.
    ...
    /* 설명. @SessionAttribute 방식으로 Session네 담긴 값은 SessionStatus에서 제공하는 setComplete()로 만료 시켜야 한다. */
    @GetMapping("logout2")
