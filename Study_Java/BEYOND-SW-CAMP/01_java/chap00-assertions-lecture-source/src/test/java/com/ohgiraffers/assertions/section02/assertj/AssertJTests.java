...
public class AssertJTests {
    ...
    /* 수업목표. test third party lib 중 AssertJ 의 사용법에 대해 이해할 수 있다. */
    ...
    /* 목차 3. 날짜 테스트하기 */
    /* 필기. 날짜 객체가 가지는 년, 월, 시, 분, 초, 밀리초, 나노초까지도 검증이 가능하며, 특정 날짜 범위, 이전, 이후 내용도 검증 가능하다. */
    @Test
    ...
    /* 목차 5. filter 를 이용한 단정문 테스트 하기 */
    /* 필기. 특정 filter 를 자바 람다식을 이용하여 표현할 수 있는 유용한 기능이다. */
    @Test
    @DisplayName("filtering assertions 테스트 하기")
    void testFilteringAssertions() {

        Member member1 = new Member(1, "user01", "홍길동", 20);
        Member member2 = new Member(2, "user02", "임꺽정", 16);
        Member member3 = new Member(3, "user03", "티니핑", 30);
        Member member4 = new Member(4, "user04", "하츄핑", 40);
        Member member5 = new Member(5, "user05", "이순신", 19);
    ...
    /* 목차 6. 객체 프로퍼티 검증하기 */
    /* 필기. 객체의 프로퍼티를 추출하여 filter 를 할 수 있다. 이 때 getter 함수가 존재하지 않아도 프로퍼티 필터링이 가능하다. */
    @Test
