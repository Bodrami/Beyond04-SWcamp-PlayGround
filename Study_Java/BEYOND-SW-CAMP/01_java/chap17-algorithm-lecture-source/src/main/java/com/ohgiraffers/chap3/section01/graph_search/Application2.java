...
/* 수업목표. BFS 와 x, y좌표를 활용한 문제를 해결할 수 있다.(feat. 배열의 인덱스와 반대 개념) */
/* 필기. 너비 우선탐색(Breadth-First Search)
 *      선입선출 구조에 queue 를 활용한다.
...
public class Application2 {
    ...
    /* 설명. 상하좌우 개념좌표 배열들 */
    static int dirx[] = {0, 0, -1, 1};             // {} 안에 순서대로: 인덱스 0번 1번 2번 3번 (x좌표)
    static int diry[] = {-1, 1, 0, 0};             // {} 안에 순서대로: 상, 하, 좌, 우 (y좌표)
    ...
    /* 설명. x 와 y 죄표를 인지하는 static 내부 클래스 */
    static class Node {
        int x;
        int y;
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) {
                ...
                if (visit[i][j] == false && map[i][j] == 1) {
                    count++;
                    bfs(j, i);      // bfs의 인자는  x좌표, y좌표 순으로 받을 것이므로 i와 j를 확인하고 넘겨야함
                }
            }
        }
