...
//@ComponentScan(basePackages = "com.ohgiraffers")  // 콩이 어디있는지 스캔해주는 것. com.ohgiraffers의 하위 폴더
...
@ComponentScan(basePackages = "com.ohgiraffers", useDefaultFilters = false, includeFilters = {
                @ComponentScan.Filter (
                        type = FilterType.ASSIGNABLE_TYPE, classes = {MemberDTO.class}
                )
            })

/* 설명. @ComponentScan은 필터의 경우 개념 정도만 알아도 되지만 기본적으로 @Configuration 클래스가 위치한 패키지와 해당 패키지 하위만 Scan
 *      패키지와 해당 패키지 하위만 Scan하기 때문에 더 상위 범위나 다른 패키기를 Scan하기 위해 주로 사용된다.  */
