...
public class OrderService {
     ...
     *      : Service 계층부터 개발할 때는 사용자가 입력한 값들이 어떻게 DTO 또는 Map으로 묶여서 Service로 넘어올 지 충분히 고민한 후 매개변수를 작성하고 개발한다.
    ...
    /* 설명. 전파행위 옵션 (트랜잭션의 동시성)
     *  REQUIRED : 진행 중인 트랜잭션이 있으면 현재 메소드를 그 트랜잭션에서 실행하되 그렇지 않은 경우 새 트랜잭션을 시작해서 실행한다.(일반적으로 가장 많이 사용)
     ...
     *                     따라서 오염된 값을 읽거나, 재현 불가능한 값 읽기, 허상 읽기 등의 문제가 발생할 수 있다. (모든 동시성 문제발생)
     ...
     *                동시성 문제는 모두 해소되지만 성능은 현저하게 떨어진다. (모든 동시성 문제는 막을 수 있다. 한글자도 틀리지 않아야 하는 예를 들어 금융권에서 사용)
    ...
    // 이 빈은 테스트 코드에도 달 수 있음 (하지만 테스트 코드에서는 테스트코드이니 커밋을 날리지 않게 롤백하는 기능으로 서비스에서 단 것이랑 의미가 다르다.)
...
//        System.out.println("menuCode = " + menuCode);
        ...
        /* 설명 4-1. insert 하기 위해 테이블과 매칭되는 Entity 로 옮겨 담는다. (DTO -> Entity) */
...
//        );
        ...
        /* 설명 4-2. 위 insert 구문을 for문으로 바꾸어 실행해보자 (위 내용이 어려우면 for문으로) */
        List<OrderMenu> oMenus = new ArrayList<>();
        ...
        for (int i = 0; i < orderMenuSize; i++) {
            ...
            orderMenu.setOrderCode(order.getOrderCode());           // 필요에 의한 setter는 추가할 수 있음
        ...
        /* 설명 2-1. 주문 건에 대한 총 합계 금액 계산 메소드 (orderMenus: 사용자의 주문내용, menus: 조회된 메뉴 전체 내용) */
