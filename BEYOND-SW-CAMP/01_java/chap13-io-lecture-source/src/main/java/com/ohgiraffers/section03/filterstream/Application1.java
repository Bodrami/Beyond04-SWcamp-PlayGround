...
public class Application1 {
    ...
    public static void main(String[] args) {
        ...
        /* 수업목표. 필터스트림이라고도 불리는 보조스트림의 BufferedWriter와 BufferedReader에 대해 이해하고 사용할 수 있다. */
        /* 필기. 내부적으로 버퍼가 활용해서 입출력의 성능을 향상 시킨다.
         *      추가적인 메소드가 제공된다.
        * */
        ...
        try {
            ...
            /* 필기. 버퍼를 이용해서 출력을 하는 경우 버퍼 공간이 가득차지 않으면 내보내기(출력)가 안되는 경우가 있다.
             *      이럴 경우 버퍼에 담긴 내용을 강제로 내보내기 위해서는 flush()를 활용해야 한다.
             *      (write를 했는데 파일에 값이 안 적혀 있으면 flush()할 것)
             *
             * 필기. stream을 close()하면 내부적으로  flush()를 호출하긴 함.
            * */
            bw.flush();
