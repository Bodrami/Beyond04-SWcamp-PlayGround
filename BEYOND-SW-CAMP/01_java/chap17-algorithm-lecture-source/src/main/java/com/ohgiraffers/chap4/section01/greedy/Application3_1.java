package com.ohgiraffers.chap4.section01.greedy;
...
public class Application3_1 {
    ...
    public static int solution(String input) throws IOException {
        ...
        int[][] time = new int[N][2];                    // (회의 갯수 * (시작 시간 + 종료 시간) 일차원 배열)인 이차원 배열
        StringTokenizer str;
        for (int i = 0; i < N; i++) {
        ...
        * */
        Arrays.sort(time, (o1, o2) -> {                  // comparator, 람다식이 사용된 이유(복습해야함):

            /* 설명. 종료시간이 같은 회의에 대해서는  */
            if (o1[1] == o2[1]){

                /* 설명. 시작 시간이 늦은 순서로(내림차순) 정렬 기준을 잡는다. (회의시간이 짧아지는 순서로 쌓기 위함) */
                return o2[0] - o1[0];
        }
            /* 설명. 종료 시간이 같지 않을 때는 종료 시간이 이름 회의부터 정렬되도록 기준을 잡는다. (종료 시간 오름차순) */
            return o1[1] - o2[1];
        });
